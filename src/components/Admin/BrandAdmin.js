...
const BrandAdmin = (props) => {
    const brand = useSelector(state => state.slider.sliderList)
    ...
    useEffect(() => {
        props.getCarouselListRequest('brand')
    }, [])
        ...
        <>
            <div className='newProduct_div'>
                <button className='newProduct'>
                    <i className="p-icon icon-plus"/> Add new brand
                </button>
            </div>
